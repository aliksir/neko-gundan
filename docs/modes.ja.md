...
モードは**どのファイルをインストールするか**を制御する。しつけは**インストール済みファイル内のどの機能を有効にするか**を制御する。

- モード = `.claude/` ディレクトリの内容
- しつけ = `neko-gundan.config.yaml` での微調整

`implement` モードをインストールした後でも、しつけでHeartbeat監視のON/OFFを切り替えられる（ファイル自体は残る）。
