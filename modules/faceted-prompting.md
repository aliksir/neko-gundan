...
This is why rules written in the middle of CLAUDE.md or agent definitions get ignored — it's not disobedience, it's attention bias.

## Neko Gundan Agent Definition Order
...
## Mapping to Neko Gundan

| Facet | Neko Gundan equivalent | Files |
|-------|----------------------|-------|
| Persona | Character & Tone section in each agent | `agents/*.md` |
| Knowledge | Active Modules, Data Source Rules | `agents/*.md` + `modules/*.md` |
...
agents/
  ├── genba-neko.md            (Agent-specific Policy at end)
  ├── shigoto-neko.md          (Agent-specific Policy at end)
  └── ...
```

Change a shared rule once in `rules/` → all agents pick it up. Agent-specific rules stay in the agent file's Policy section.
