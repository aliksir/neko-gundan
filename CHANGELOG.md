...
### Added
- **Gate-guard date freshness check** (`isRecentFile`): plans/checklist files with YYYYMMDD prefix older than 3 days are ignored, preventing stale artifacts from falsely satisfying the gate
- **Evidence crosscheck CLI** (`neko-evidence-crosscheck.mjs`): one-command verification that all required artifact types (8 for squad, 9 for platoon) exist for a given task slug

### Changed
- NEKO_WORK_DIR environment variable support in 4 scripts (check-wiring/drift-check/flow-consistency-check/session-hours): fallback chain `env || argv || cwd` replaces hardcoded `C:/work`
- .publish-deny-exempt: removed 4 completed NEKO_WORK_DIR entries, updated comments

### Fixed
- evidence-crosscheck: multi-stage match for metrics (standard → date-stripped → longest-prefix reverse lookup) with stderr warning for ambiguous prefix matches
- flow-consistency-check: eliminated BASE.replace reverse-calculation logic (unescaped regex bug), now constructs paths directly from WORK_DIR
- check-wiring.mjs: added HOOK_EXEMPT_FILES for 4 legitimately exempt hook files (mcp-response-inspector dual-registration avoidance)
- publish-deny-exempt: added dashboard.md and README.md to resolve GLOBAL_DENY collision
- plugin.json version bump to 1.12.1 + /neko-model command entry alignment

## [1.12.0] - 2026-07-06

### Added
- **Kantoku (監督猫) v5 3-layer architecture**: Layer 1 gate-guard extension + Layer 2 watchdog. Batch 1a/1b ledger CLI (stats/gate-check/end-check/final-report), AGENT_STOP final-report auto-append, declare_late 2-call limit enforcement
- **Trust Propagation Graph (TPG)**: v1-v3 with trust score decay, cycle detection, ISV drift event integration, handoff_schema v3.3 trust_context field
- **Intent Drift Detector (IDD)**: ISV Phase 4 analysis integration
- **Mutation Review module**: mandatory for development tasks, Knowledge Harvesting section
- **Flow consistency checker**: 7 categories, 48 items for workflow-flow.html validation
- **Deliverable quality checker** (`deliverable-check.mjs`): mechanical verification of deliverable quality
- **Staleness checker** (`check-staleness.mjs`): holdover task staleness detection via completion ledger
- **Check-wiring.mjs hook audit**: settings.json registered path vs actual hook file cross-validation
- **Gate-guard checklist auto-block**: reviewing phase blocks result/ write when checklist has unchecked items
- **Publish deny-exempt mechanism** (`.publish-deny-exempt`): per-repo exemption for global deny patterns
- **KANTOKU_EXCLUDE_PATTERNS**: publish whitelist auto-excludes kantoku-* files
- **GLOBAL_DENY_PATTERNS**: publish pre-scan for local-only paths and kantoku file references
- **scripts/README.md**: importance-based classification guide
- **`/neko-model` command**: per-agent model override (opus/sonnet/haiku individual or bulk assignment, `reset` to inherit session model)

### Changed
- **NEKO_WORK_DIR**: `phase-context.mjs` and `kantoku-stop.mjs` now use `NEKO_WORK_DIR` env var (default: `C:/work`)
- **Gate item counts**: aligned to actual gate definitions (nano:6/mini:12/full:17/mini-complete:21/full-complete:28)
- **Phase transitions**: all disallowed transitions now hard-blocked (null->implementing loophole closed)
- **Commit-guard**: taskName-based artifact search, scripts/commit-guard.mjs removed (consolidated into production hook)
- **Adapters and audit-trail templates**: integrated from other-neko-gundan

### Fixed
- Audit quick wins: CI hardening, bug fixes, README refresh
- Config naming unification, Fable refs fix, hooks docs
- SC2155 shellcheck warning in lifecycle.sh
- Commit-guard recon exclusion + artifact-check improvements
- artifact-check.sh glob improvement + mtime filter + metrics header token
- Hook extension filter unification + designing phase skip block
- Drift log recording before phase block + extension detection tests
- Local paths re-scrub (NEKO_WORK_DIR/CLAUDE_CONFIG_DIR) + forward-only phase auto-set guard
- taskName change now resets reviews (critical multi-reviewer consensus bug fix)
- kantoku-stop.mjs `remove` command removed (critical safety audit finding)
- Publish deny-exempt: dashboard.md and README.md added to exempt list
- session-hours time delimiter fix + plan-template status section
- gate-guard.mjs production sync (stale context detection + done block)

## [1.11.2] - 2026-05-31

### Fixed
- **nightly-runner personal paths parameterized (completes the 1.11.1 deferral)**: `scripts/nightly-runner.mjs` and `rules/nightly-policy.yml` hardcoded the maintainer's local home/work paths — explicitly deferred in 1.11.1 as "runtime code kept out of scope to avoid breaking behavior". A new exported pure function `resolvePaths(env, repoRoot)` now derives every path from environment variables (`NEKO_WORK_DIR`, `NEKO_PROFILE_PATH`, `NEKO_LOG_DIR`, `NEKO_MEMO_YOSHI_DIR`, `CLAUDE_CONFIG_DIR`) following the existing hooks convention. Defaults derive from `REPO_ROOT`'s parent and `HOME`/`USERPROFILE`, resolving to the previous paths with **zero behavior change** (verified by dry-run) while removing all username and work-dir literals from the published runtime code. `nightly-policy.yml` prompt templates use `{WORK_DIR}` / `{MEMO_YOSHI_DIR}` / `{CLAUDE_CONFIG_DIR}` / `{LESSONS_DIR}` placeholders expanded at runtime. Tests moved to neutral fixtures + 4 new cases (env override, deterministic default derivation, backslash normalization, default-path sanity). All 16 unit tests pass.

### Documentation
- **Dynamic Workflows "How to activate" note (EN/JA)**: The README described the capability but not how a user triggers it. Added a note — explicit opt-in via the `workflow` keyword or `/effort ultracode`, with graceful TeamCreate fallback where the research-preview feature is unavailable.

## [1.11.1] - 2026-05-31

### Documentation
- **README.koneko EN/JA parity**: Added Dynamic Workflows to the koneko Limitations section as a full-Neko-Gundan-only feature, and restored the missing "v1.10.x heavy features" line in `README.koneko.ja.md` so the EN/JA limitation lists match (5 items each).

### Changed
- **Generalized local-specific vocabulary across 18 files**: Replaced hardcoded local paths (`C:/work/...` in docs/YAML examples → relative or `<placeholder>`) and personal project names in usage examples (`jp-dashboard`/`x-auto-bot`/`ses-finder`/`react-board-app` → generic `my-app`/`api-server`/etc.). Official ecosystem tool names (`neko-kensa`/`code-factory`) keep their names with only the path prefix genericized (`<neko-kensa>`/`<code-factory>`); world-building terms (親方猫/総司令/etc.) are preserved. Runtime code (`nightly-runner.mjs` / tests / `nightly-policy.yml`) and CHANGELOG history are intentionally kept out of scope to avoid breaking behavior.
