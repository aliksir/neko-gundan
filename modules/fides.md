...
## Trust Propagation Graph (TPG) Integration

When the `trust_propagation_graph` module is active (Platoon+ scale), FIDES trust tags are dynamically tracked via a DAG:

- **PostToolUse**: Every tool result is recorded as a graph node with its trust level
- **PreToolUse (Bash)**: Before executing commands, recent graph nodes are checked for LOW contamination
- **Propagation rule**: Output trust = min(input trust levels) — weakest-link principle
- **Promotion**: LOW → MEDIUM promotion via FIDES procedure is recorded as `promoted: true`

See `modules/trust-propagation-graph.md` for full specification.
