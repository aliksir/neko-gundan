...
cmd_after_create() {
    ...
    if [ -x "$TOKEN_TRACKER" ]; then
        if [ -n "${team}" ]; then
            bash "$TOKEN_TRACKER" add-agent "$team" "$agent" 2>/dev/null || \
                echo "[lifecycle] WARNING: token-tracker add-agent 失敗 (team=${team}, agent=${agent})" >&2
        else
            echo "[lifecycle] WARNING: team未設定のため token-tracker add-agent をスキップ (agent=${agent})" >&2
        fi
    fi
...
cmd_on_stall() {
    ...
    mkdir -p "$alert_dir"
    local alert_file
    alert_file="$alert_dir/stall-${agent}-$(date '+%Y%m%d%H%M%S').json"
    cat > "$alert_file" <<EOF
{
  "type": "stall",
