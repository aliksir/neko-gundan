...
| 判断 | 仕組み | いつ | 例 |
|------|--------|------|---|
| **何を入れる？** | [モード](docs/modes.ja.md) | `install.sh` | `quality+security` |
| **どこまで回す？** | [プロセスウェイト](docs/process-weight.ja.md) | タスクごと | 「ライトで」「厳密に」 |
| **何をON/OFFする？** | [しつけ](docs/shitsuke-guide.md) | `config.yaml` | `heartbeat: false` |
| **何が常に守られる？** | 安全対策 | 常時 | 削除安全策、競合防止 |

```
install.sh          タスクごとのキーワード   config.yaml         常時有効
┌──────────┐       ┌──────────────┐       ┌───────────┐       ┌──────────┐
...
└──────────┘       └──────────────┘       └───────────┘       └──────────┘
 初回セットアップ     タスクごとに変更        いつでも調整       絶対に緩めない
```
