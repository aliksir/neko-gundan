...
handoff:
  ...
  blockers:          # Blockers (if any)
    - string
  trust_context:     # Optional (TPGモジュール有効時のみ)
    effective_trust: enum    # "HIGH" | "MEDIUM" | "LOW" — ハンドオフ時点の実効信頼レベル
    low_node_count: integer  # 直近ウィンドウ内のLOWノード数（0以上）
    contamination_path: bool # 汚染パス（LOW→HIGH伝搬）の有無
    tpg_snapshot_id: string  # TPGグラフのスナップショットID
```
...
9. `trust_context` present: `effective_trust` must be one of HIGH/MEDIUM/LOW
10. `trust_context` present: `low_node_count` must be integer >= 0
11. `trust_context` present: `contamination_path` must be boolean
12. `contamination_path: true` implies `effective_trust` should be LOW (warning if mismatch)
13. `trust_context` present: `tpg_snapshot_id` must be non-empty string
...
| genba-neko | Post-work (TPG active) | Populate `trust_context` from TPG session state (effective_trust, low_node_count, contamination_path, tpg_snapshot_id) |
| shigoto-neko | Handoff review | Validate handoff fields (from/to required, status valid, completed non-empty, action field appropriate); If trust_context present, verify rules 9-12 |
| shigoto-neko | Task routing | Use `action` field to decide: auto (proceed), confirm (approve first), propose_only (review only). If `contamination_path: true`, override to `confirm` regardless of action field |
...
## Trust Context と FIDES trust_level の関係

| 観点 | FIDES `trust_level` | TPG `trust_context` |
|------|---------------------|---------------------|
| 粒度 | 個別データの静的信頼タグ | セッション全体の動的信頼伝搬サマリ |
| 設定タイミング | データ取得時に即時タグ付け | ハンドオフ時に TPG グラフから集計 |
| 依存関係 | FIDES が入力 | TPG が FIDES のタグを伝搬追跡した結果 |
| 共存 | 両方存在可能 | trust_context.effective_trust はセッション全体の最小信頼レベル |

`trust_context` は Optional。TPG モジュールが OFF の場合は省略される。FIDES の `trust_level` は TPG の有無に関わらず独立して機能する。
