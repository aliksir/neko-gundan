...
Example:
```
- [api-server] [learned] External API rate limit is 15 req/15min per app token — Use a user token for higher limits (example)
- [web-app] [constraint] Vite dev server HMR fails with symlinked node_modules — Use --preserve-symlinks flag (example)
- [mcp-server] [rejected] zod validation for MCP responses — Too heavy for real-time filtering, use regex pattern matching instead (example)
```
