...
[oyakata-neko]   GATE   完了ゲート — 成果物 5 種揃い、CR-1〜CR-5 違反 0
...
| 連隊 | 10ファイル以上、プロジェクト横断 | 仕事猫x3 + 現場猫x5 |

デフォルトではセッションのモデルをそのまま継承する（model未指定）。`/neko-model` コマンドでエージェントごとに上書きできる:
...
### Mutation Review（レビュー品質検証）

kurouto-neko が APPROVE を出した後、承認済みコードに意図的なバグを注入してレビュアー自身の品質を検証する。ブラインドレビューで何件検出できるかを測定し、RQS（Reviewer Quality Score）として記録。80% 以上で PASS、60-79% はレビュー設定の改善が必要、60% 未満はタスク即停止。小隊+ のコード変更時は必須。詳細: [`modules/mutation-review.md`](modules/mutation-review.md)

### Trust Propagation Graph (TPG)

エージェント間でデータの信頼度がどう伝搬するかを追跡する。高信頼ソース（コマンダー入力・プロジェクトファイル）からの派生出力は信頼スコアを引き継ぎ、低信頼ソース（外部 API・Web スクレイピング）はパイプライン全体に LOW タグを伝搬する。循環依存は cycle detection で検出。v3: ISV ドリフトイベントとの統合。詳細: [`modules/trust-propagation-graph.md`](modules/trust-propagation-graph.md)

### Intent Drift Detector (IDD)

セッション中にエージェントが当初のタスク目的から逸脱していないかを監視する。ISV（Intent State Vector）Phase 4 分析と統合し、ドリフト角度が閾値を超えると誤方向での作業継続前に警告を出す。詳細: [`modules/intent-drift-detector.md`](modules/intent-drift-detector.md)
...
**起動方法:** この機能はユーザーが明示的にオプトインしたときだけ作動する。リクエストに `workflow` という語を含めるか、ultracode を有効にする（`/effort ultracode`）。オプトインしない場合、Neko Gundan は通常の TeamCreate サブエージェントで統括し、挙動は変わらない。Dynamic Workflows は Claude Code のリサーチプレビュー機能で、Workflows を同梱する Claude Code バージョンが必要。未対応環境では TeamCreate に自動フォールバックする。
...
他のAIコーディングCLIでも猫軍団のルールを使える:

| CLI | ファイル | 使い方 |
|-----|---------|--------|
| [Codex CLI](https://github.com/openai/codex) | `adapters/codex/AGENTS.md` | プロジェクトルートに `AGENTS.md` としてコピー |
| [Aider](https://github.com/paul-gauthier/aider) | `adapters/aider/conventions.md` | プロジェクトルートに `CONVENTIONS.md` としてコピー |
| [Gemini CLI](https://github.com/google-gemini/gemini-cli) | `adapters/gemini/GEMINI.md` | プロジェクトルートに `GEMINI.md` としてコピー |

各アダプタには猫軍団のコアルール（実装者 != レビュアー、3サイクル上限、証跡ベースのゲート）が、各CLIの読み取り形式で記載されている。
...
| [YAML定義](yaml/) | エージェント・ルール・モジュール・ゲートの機械可読YAML版（実験的、.md 正本から遅延する場合あり） |
