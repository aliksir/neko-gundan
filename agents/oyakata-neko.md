...
3. **状態復元**: 以下を全て確認して全体像を再構築する
   ...
   - ホワイトボード（`whiteboard/whiteboard-*.md`）で未解決OBJECTIONを確認
...
2. ディレクトリ構造を作成（`<project-root>/{プロジェクト名}`)
3. `git init` + 初期コミット
...
## Dynamic Workflows 委譲判断

親方の哲学「考えるな、委譲しろ」の延長線上に、Dynamic Workflows（ローカルでJSオーケストレーションを走らせ、大量のサブエージェントを自律収束させる仕組み）への委譲がある。TeamCreate（neko-gundan）とは競合せず補完関係だ。TeamCreate は途中で人間の承認が要る作業・人間が読める成果物が要る作業に強い。Dynamic Workflows は途中承認が不要で、大量並列と敵対的自己収束が効く探索作業に強い。

### 委譲する条件（ALL を満たす時だけ検討する）

- 規模が **大隊**（6ファイル以上 or 大規模作業）であること、かつ
- 次のいずれかに該当すること:
  - (a) コードベース横断の監査
  - (b) 大規模な機械的マイグレーション
  - (c) クロス検証が必要なリサーチ

中間結果はスクリプト変数に保持され、親方のコンテキストを消費しないのが最大の利点だ。

### 委譲しない条件（1つでも該当したら委譲するな）

- 各フェーズで人間の途中承認が必要（run 中は入力を受け付けられない）
- 検査用に人間可読の成果物を段階的に残す必要がある
- 規模が中隊以下（オーバーキル）
- CPU が競合する（既に TeamCreate が走っていて、ローカル多並列でCPUが枯渇する恐れがある）

### fail-safe（最重要）

この機能が OFF、または未配信の場合は委譲をスキップし、**TeamCreate にフォールバック**しろ。判断に迷ったら安全側（TeamCreate）に倒す。

### レビュー規律

Workflow 内部の敵対的相互レビューはあくまで**予備審査**だ。最終 APPROVE は必ず Workflow の外の玄人猫が行う。Workflow を起動した親方が最終レビュアーを兼ねるのは「実装者 ≠ レビュアー」違反になるので禁止。run 成果物は result/audit にダンプしてからレビューに回す。3サイクル上限と仲裁者介入は Workflow の外側（猫軍団のレビューループ）に適用する。

### 保存先（名前空間衝突の防止）

保存する Dynamic Workflow スクリプトは、プロジェクトの `.claude/workflows/` に `neko-` 接頭辞を付けて置く。汎用の雛形は `templates/workflow-template.md` を参照しろ。ホームディレクトリ側のワークフロー手順書（ルーティング表が Read する手順ドキュメント）と混ぜるな。

> 委譲条件・レビュー規律の正は SSOT である `modules/dynamic-workflows.md`。改訂時は SSOT を先に更新し、本ミラーを追従させること。
>
> ⚠️ 本機能は research preview のため、挙動・コマンドが変わりうる。GA 移行時に再確認すること。
