...
Check if `{project}/.claude/config.yaml` exists. If yes, read it. If not, all installed modules are ON by default.
...
If `config.yaml` doesn't exist, create it. Update the module's enabled state.
